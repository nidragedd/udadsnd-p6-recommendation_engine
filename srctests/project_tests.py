    ...
    if sol_1_dict_ == sol_1_dict:
        ...
        for k, v in sol_1_dict.items():
            if sol_1_dict_[k] != sol_1_dict[k]:
                print("Oops! It looks like the value associated with: {} wasn't right. Try again.  It might just be "
                      "the datatype.  All of the values should be ints except the article_id should be a string.  "
                      "Let each row be considered a separate user-article interaction.  If a user interacts with an "
                      "article 3 times, these are considered 3 separate interactions.\n\n  Notice you may also find "
                      "the number of unique users as 5149 if you count the null user.  However, this is hard to catch "
                      "without mapping first!".format(k))
...
def sol_2_test(top_articles):
    ...
    for idx, file in enumerate(checks):
        if set(eval(file)) == set(pickle.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                                "{}.p".format(file)), "rb"))):
    ...
    if sol_5_dict == sol_5_dict_1:
        ...
        for k, v in sol_5_dict_1.items():
            if set(sol_5_dict[k]) != set(sol_5_dict_1[k]):
                print("Oops!  Looks like there is a mistake with the {} key in your dictionary.  The answer should"
                      " be {}.  Try again.".format(k, v))
...
def sol_4_test(sol_4_dict):
    ...
    if sol_4_dict == sol_4_dict_1:
        print("Awesome job!  That's right!  All of the test movies are in the training data, but there are only 20 "
              "test users that were also in the training set.  All of the other users that are in the test set we have"
